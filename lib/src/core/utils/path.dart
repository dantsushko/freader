...
Future<String> getDir([List<String> pathSegments = const []]) async {
  final absoulte = (await getApplicationDocumentsDirectory()).absolute;
...
String getDirName(String entityPath) =>
    FileSystemEntity.typeSync(entityPath) == FileSystemEntityType.file
        ? path.basename(path.dirname(entityPath))
        : path.basename(entityPath);
String getFileExtension(String filePath) => path.extension(filePath.replaceAll('.zip', ''));
String getRelativeBookPath(String filePath, String dirName) =>
    baseBookDirName + '/' + dirName + '/' + getFileName(filePath);

BookFormat getBookFormat(String filePath) =>
    BookFormat.values
        .firstWhereOrNull((e) => e.name == getFileExtension(filePath).replaceAll('.', '')) ??
